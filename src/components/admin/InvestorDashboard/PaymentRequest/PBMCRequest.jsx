...
function PBMCRequest() {
    ...
    async function sendETH({ target }) {
        const _id = target.getAttribute('data-id')
        const index = target.getAttribute('data-value')
        setLoading(index)
        const address = document.getElementsByClassName('eth_walletAddress')[index].getAttribute('address')
        const amount = document.getElementsByClassName('ethAmount')[index].getAttribute('amount')
        // console.log({ address, amount });
        const EthValue = Web3.utils.toWei(amount);
                                                        ...
                                                        laoder ?
                                                            ...
                                                            allPaymentData.length > 0 ?
                                                                (rowsPerPage > 0
                                                                    ...
                                                                    return (
                                                                        <TableRow key={i}>
                                                                            <TableCell>{e.payment_type === "PBMC" ? e.transactionId.slice(0, 6) + "......" + e.transactionId.slice(-6) : e.transactionId}</TableCell>
                                                                            <TableCell className='eth_walletAddress' address={e.walletAddress}>{e.walletAddress}</TableCell>
                                                                            <TableCell>{e.received_currency}</TableCell>
                                                                            <TableCell className='ethAmount' amount={e.pbmc_amount.toFixed(3)}>{e.pbmc_amount.toFixed(3)}</TableCell>
                                                                            <TableCell>
                                                                                {e.action_status === true ?
                                                                                    e.transaction_hash !== undefined &&
                                                                                    <a target='_blank' href={`https://sepolia.etherscan.io/tx/${e.transaction_hash}`}>
                                                                                        {`${e.transaction_hash.slice(0, 4)}...${e.transaction_hash.slice(-4)}`}
                                                                                        {/* {
                                                                                            console.log("JSX",e)
                                                                                        } */}
                                                                                    </a>
                                                                                    ...
                                                                                    :
                                                                                    <button data-id={e._id} data-value={i} className='btn btn-primary' onClick={sendETH} style={{ padding: "2px 10px" }}>Send</button>
                                                                                }
                                                                            </TableCell>
                                                                        </TableRow>
                                                                    )
