...
function Supplier_list() {
    ...
    const emptyRows =
        page > 0 ? Math.max(0, (1 + page) * rowsPerPage - supplierData.length) : 0;
    ...
    const getSuppliers = async () => {
        await axios.get(`${process.env.REACT_APP_BASE_URL}/admin/getAllDetailsSupplier`,
            {
                headers: {
            ...
            console.log("admin supplierData list res---", res.data)
            setLoader(false)
            setSupplierData(res.data.details.reverse())
            // setTotalCount(res.data.count)
        })
            .catch((err) => {
                console.log("admin supplierData list err", err)
                setLoader(false)
            })
    }
