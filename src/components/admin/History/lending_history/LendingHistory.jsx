...
function LendingHistory() {
    ...
    const [userLendingData, setUserLendingData] = useState([])
    const [laoder, setLoader] = useState(true)
    // Avoid a layout jump when reaching the last page with empty rows.
    const emptyRows =
        page > 0 ? Math.max(0, (1 + page) * rowsPerPage - userLendingData.length) : 0;
    ...
    const getUserLendingData = async () => {
        await axios.get(`${process.env.REACT_APP_BASE_URL}/funds/getloandetails/${loginData._id}`,
            {
                headers: {
            ...
            // console.log("user lending list res---", res.data)
            setLoader(false)
            setUserLendingData(res.data.details.reverse())
        })
            .catch((err) => {
                console.log("user lending list err", err)
                setLoader(false)
            })
    }
                                        ...
                                        <Table aria-label="custom pagination table">
                                            <TableHead>
                                                <TableRow>
                                                    ...
                                                    <TableCell>Colleteral Amount</TableCell>
                                                    <TableCell>LTV(%)</TableCell>
                                                    <TableCell>Received Funds</TableCell>
                                                    <TableCell>Interest Rate(monthly)</TableCell>
                                                </TableRow>
                                            </TableHead>
                                            <TableBody>
                                                {
                                                    laoder ?
                                                        ...
                                                        :
                                                        userLendingData.length > 0 ?
                                                            (rowsPerPage > 0
                                                                ? userLendingData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                                                                : userLendingData
                                                            ).map((e, index) => (
                                                                <TableRow key={index}>
                                                                    <TableCell>{page * rowsPerPage + index + 1}</TableCell>
                                                                    <TableCell><div style={{ textTransform: "uppercase" }}>{e.received_currency} {e.payment_type}</div></TableCell>
                                                                    <TableCell>{e.loan_percentage}</TableCell>
